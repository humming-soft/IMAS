...
Class ProgrammeModel extends CI_Model
{
	...
	function getAllProgrammes($uid,$status,$rid=''){
		$this->db->select('t1.prog_id, t1.prog_name, t1.prog_desc, t1.proc_value, t2.cur_code_name, t3.cur_scale_name, t4.proc_agcy_name, t5.proc_agcy_sec_name, t1.prog_progress_status, t1.proc_agency_id, t1.proc_sector_id, t1.created_at, t1.prog_start_date, t1.prog_end_date');
	...
	function updateProgramme($id, $data){
		...
		$this->db->update('tbl_programmes',$data);
	...
	function getProgrammeById($id,$uid,$status,$rid=''){
		$this->db->select('t1.prog_id, t1.prog_name, t1.prog_desc, t1.proc_value, t1.currency_code_id, t1.currency_scale_id, t2.cur_code_name, t3.cur_scale_name, t1.proc_agency_id, t4.proc_agcy_name, t1.proc_sector_id, t5.proc_agcy_sec_name, t1.prog_progress_status, t1.proc_agency_id, t1.proc_sector_id, t1.created_at, t1.prog_start_date, t1.prog_end_date');
		$this->db->from('tbl_programmes as t1');
	...
	function getProgrammeByName($name,$uid,$status,$rid=''){
		$this->db->select('prog_id, prog_name, prog_desc, proc_value, currency_code_id, currency_scale_id, proc_agency_id, proc_sector_id, prog_start_date, prog_end_date');
...
//        print_r($query);
	...
	public function get_non_progressive_journals($search, $offset, $perPage, $order, $by, $userid,$roleid) {
		$search=strtolower($search);
		$query = "SELECT * FROM (SELECT a.project_name,b.nonp_enabled,b.journal_name, b.project_no, b.reminder_frequency ,b.journal_no,e.user_full_name, e.user_id AS owner_user_id, jvn.validate_user_id, jdu.data_user_id 
					FROM project_template a, journal_master_nonprogressive b, sec_user e, journal_validator_nonprogressive jvn, 
							journal_data_user_nonprogressive jdu 
					WHERE a.project_no=b.project_no 
						AND b.user_id=e.user_id 
						AND jvn.journal_no=b.journal_no 
						AND jdu.journal_no=b.journal_no ";
		if($userid != "1" && $roleid !="1"){
			$query .=" AND b.journal_no IN (SELECT journal_no FROM journal_data_user_nonprogressive WHERE data_user_id=$userid)";
		}
		
		if($search != "")
		{
			$query .= " AND (lower(a.project_name) like '%".$search."%' or lower(b.journal_name) like '%".$search."%' )";;
		}
		$query .= ") a1";
	...
	public function get_non_progressive_journals_attr(){
		return array(
			"journal_no",
			"project_name",
			"journal_name",
