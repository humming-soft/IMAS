...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', user_id, user_password, user_status, last_login')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->user_password, $identity))
			{
				...
				// Rehash if needed
				$this->rehash_password_if_needed($user->user_password, $identity, $password);
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			{
				$this->db->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.role_id', 'inner');
				$this->db->where_in($this->tables['groups'] . '.role_name', $group_names);
			}
			if(!empty($group_ids))
			{
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.role_name, '.$this->tables['groups'].'.role_desc')
	...
	public function in_group($check_group, $id = FALSE, $check_all = FALSE)
	{
		...
		else
		{
			...
			{
				$groups_array[$group->id] = $group->role_name;
			}
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], ['last_login' => time()], ['user_id' => $id]);
